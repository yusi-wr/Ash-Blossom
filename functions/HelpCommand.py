...
import nextcord
from config import COLOR_EMBED, IMAGE_LINK
...
class MyHelpCommand(commands.MinimalHelpCommand):
    ...
    async def _cog_select_options(self) -> list[nextcord.SelectOption]:
        ...
        options.append(nextcord.SelectOption(
            label="Home",
            emoji="<:General_Info:1191515870011342948>",
    ...
    async def _help_embed(
        ...
        embed = Embed(title=title, color=COLOR_EMBED)
        if description:
        ...
        if set_author:
            ...
            embed.set_image(url=IMAGE_LINK["image-help2"])
        if command_set:
